...
df = pd.read_excel("event.xlsx")
emails = df['Emails'].values.flatten()
naams = df["Name"].values.flatten()
df.index = naams
...
for names in tqdm_gui(file['Name']):
    ...
    image.save('certificates/'+names+'.pdf')

    # for sending the email
    senderEmail = 'shahzaneer.dev@gmail.com'
    senderPass = 'ENTER_Your_Password_Here'
    subject = "Certificate of Participation CS-Lodges"
    bodyEmail = "Greetings!\n Hope this Email Finds you in the best of Health and Spirits!\n This is Shahzaneer Ahmed , Community Lead CS Lodges, We Thank you for your Participation in Event Organized by CS Lodges. We believe that it was an awesome Learning experience and are eager to arrange such events in the future as well. Thank you for being with us\n Shahzaneer Ahmed\n CS-Lodges\n shahzaneer.dev@gmail.com"
    filename = names
    location = "E:\CSLodgesCertificate\certificates/"
    searchedEmails = df.Emails[names] # The beauty of Pandas >> I Love pandas 🎈
    send = sendpdf(senderEmail,searchedEmails,senderPass,subject,bodyEmail,filename,location)
    send.email_send()

    print(f"Email Send to %s at Email Address : %s"%(names,searchedEmails))
